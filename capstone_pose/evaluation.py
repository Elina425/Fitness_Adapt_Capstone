...
def intervals_to_mask(times_sec: np.ndarray, intervals: List[Interval]) -> np.ndarray:
    """
    Returns boolean mask for frames whose time lies in any interval.
    """
...
def knee_angle_separation_score(
    ...
    """
    Computes a simple separation metric between error vs non-error frames:
    abs(mean_error - mean_non_error) / (std_all + eps)
    """
